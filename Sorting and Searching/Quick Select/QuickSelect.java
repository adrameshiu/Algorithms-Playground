...
public class QuickSelect {
    ...
    static int select(int[] A, int k) {
        int kIndex = subSelect(A , 0 , A.length-1 , k);
        return  kIndex;
    }

    static int subSelect(int[] A, int lo, int hi , int k) {
        if(lo == hi)
            return A[hi];
        ...
        if(k == pivot)
            ...
            return subSelect(A, lo , pivot - 1 , k); //till pivot index
        else
            return  subSelect(A, pivot + 1, hi , k ); //after pivot index
    }
